...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>traubloxfruit chào ae<Color=/>"):Display()
    wait(.5)
    ...
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Đăng Ký Kênh YouTube Traubloxfruit để xem video mới nhất nhé<Color=/>"):Display()
    wait(.36)
end)

local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
local Window = OrionLib:MakeWindow({Name = "traubloxfruit", HidePremium = false, IntroText = "traubloxfruit⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
OrionLib:MakeNotification({
    Name = "traubloxfruit",
    Content = "Đăng Ký Kênh YouTube Traubloxfruit nhé💕",
    Image = "rbxassetid://8192162908",
    Time = 10
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "traubloxfruit(Noitification)",
